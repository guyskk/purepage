...
def check_captcha(token, text, key, salt):
    try:
        text2 = decrypt(token, key, salt)
...
# salt, exp
CAPTCHA_SALT = None, None
CHARS = string.ascii_letters + string.digits


def get_salt():
    """返回salt。如果salt过期了，更新salt。"""
    global CAPTCHA_SALT
    salt, exp = CAPTCHA_SALT
    if not salt or datetime.utcnow() > exp:
        salt = gen_salt(16)
        sec = current_app.config["CAPTCHA_SALT_EXP"]
        exp = datetime.utcnow() + timedelta(seconds=sec)
        CAPTCHA_SALT = salt, exp
    return salt
...
def check(token, text):
    ...
    salt = get_salt()
    return check_captcha(token, text, key, salt)
...
class Captcha(Resource):
    ...
    def get(self):
        ...
        token = encrypt(text, key, get_salt())
    ...
    def get_show(self, token):
        ...
        token = token.encode("ascii")
        try:
            text = decrypt(token, key, get_salt())
